...
import { recordSkipped, reserveEmissionIfNeeded, updateSubmitted } from '../db/emissions.js';
...
export const submitForPeriod = async (periodId: number): Promise<void> => {
  ...
  // Idempotent reservation prior to compose/send
  if (!reserveEmissionIfNeeded(periodId)) return;
